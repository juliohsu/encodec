...
class SEANetEncoder(nn.Module):
    ...
    Args:
        ...
        ratios (Sequence[int]): kernel size and stride ratios. (The encoder uses downsample ratio and its normal order)
        activation (str): Activation function.
        activation_params (dict): Parameters to provide to the activation function
        norm (str): Normalization method.
        norm_params (dict): Parameters to provide to the underlying normalization used along with the convolution.
        ...
        true_skip (bool): Whether to use true skip connection or a simple
            convolution (streamable) as the skip connection in the residual network blocks.
        compress (int): Reduced dimensionality in residual branches (from Demucs v3).
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 pad_mode: str = 'reflect', true_skip: bool = True, compress: int = 2, lstm: int = 2):
        super().__init__()
    ...
    def forward(self, x):
        print(f'ENCODER MODEL ARCHITECTURE ({len(self.model)} LAYERS)')
        print(self.model)
        return self.model(x)
...
class SEANetDecoder(nn.Module):
    ...
    Args:
        ...
        n_filters (int): Base width for the model.
        n_residual_layers (int): nb of residual layers.
        ratios (Sequence[int]): kernel size and stride ratios. (The decoder uses upsample ratio and its reverse order)
        activation (str): Activation function.
        activation_params (dict): Parameters to provide to the activation function.
        final_activation (str): Final activation function after all convolutions.
        final_activation_params (dict): Parameters to provide to the final activation function.
        norm (str): Normalization method.
        ...
        true_skip (bool): Whether to use true skip connection or a simple
             convolution (streamable) as the skip connection in the residual network blocks.
        compress (int): Reduced dimensionality in residual branches (from Demucs v3).
        lstm (int): Number of LSTM layers at the end of the decoder.
    ...
    def forward(self, z):
        print(f'\nDECODER MODEL ARCHITECTURE ({len(self.model)} LAYERS)')
        print(self.model)
        y = self.model(z)
        return y
